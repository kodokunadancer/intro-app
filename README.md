...
開発環境(Docker)で作成した自前のイメージをECRへプッシュさせ、それをECSを使用してコンテナ化しました。  
...
## 1-5 すべての工程を独学で（簡易的ではあるが）作りきったこと  
...
この差異によってDockerfileを調整したり、新たにインストールしなければならないものもあり、その対応が結構大変でした。  
  
**SPA構築**  
  
アプリを完全にSAP化するには、思ったよりも多いコード量が必要で最初は苦労しました。  
しかしやっていくうちに、コンポーネント間のデータのやりとりや、Vuexを用いたデータの保管など、うまく対応できるようになりました。  
